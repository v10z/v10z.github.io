(function () {
  ...
  Streams.History.isSupported = function () {
    return window.history && window.history.pushState;
  }

  Streams.History.prototype = {
    ...
    insert: function () {
      ...
      history.replaceState({
        ...
        // Set on 404/403/50x
        isError: Streams.History.isErrorPage,
      }, title, url);
    ...
    push: function (url, title, scroll, reverse) {
      ...
      this.insert();
      Streams.History.isErrorPage = false;
    ...
    applyBehavior: function (el) {
      ...
      if (history.state && history.state.depth > 0) {
        ...
        if (history.state.back) {
          title = history.state.back.replace(' â€Streamsko.net', '');
